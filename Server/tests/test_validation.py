...
def test_comment_user_id_validation():
    with app.app_context():
        with pytest.raises(ValueError, match="User id must be a valid user"):
            Comment(user_id=999, post_id=1, content="valid_content")

def test_comment_post_id_validation():
    with app.app_context():
...
def test_comment_like_user_id_validation():
    with app.app_context():
        with pytest.raises(ValueError, match="User id must be a valid user"):
            CommentLike(user_id=999, comment_id=1)

def test_comment_like_comment_id_validation():
    with app.app_context():
